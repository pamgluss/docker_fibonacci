...
class DjangoFibCalculator extends Component {
    state = {
        seenIndices: [],
        values: {},
        index: ''
    ...
    async fetchValues() {
        const values = await axios.get('/api2/values/current');
        this.setState({
            values: values.data
        })
    }
    ...
    async fetchIndices(){
        const indices = await axios.get('/api2/values/all');
        this.setState({
...
export default DjangoFibCalculator;
